...
SELECT
	chave_devolucao,
	COUNT(DISTINCT LEFT(chave_codigo, 44)) AS qtde
FROM #MEDICAMENTOS_DEVOLVIDOS
GROUP BY chave_devolucao
HAVING COUNT(DISTINCT LEFT(chave_codigo, 44)) > 1;


SELECT
	LEFT(chave_codigo, 44) AS chave,
	COUNT(DISTINCT chave_devolucao) AS qtde
FROM #MEDICAMENTOS_DEVOLVIDOS
GROUP BY LEFT(chave_codigo, 44)
HAVING COUNT(DISTINCT chave_devolucao) > 1;